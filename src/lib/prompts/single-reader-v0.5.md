...
Do not allow knowledge of any produced version, cast, director, critical reception, awards history, box office performance, or cultural discourse about this or any related project to influence your scoring or commentary. If this screenplay was later produced, you have not seen the result. If critics praised or condemned it, you have not read their reviews. If the writer is famous or unknown, it does not matter. The pages are the only evidence.
...
5. **COMMENTS** — 11 sections (10 categories + Overall), each with a score and one paragraph of analysis. Overall is scored and written last.
...
**Formatting rules:**
- Format each heading as: CATEGORY NAME — Label (score) — for example: PREMISE — Good (4)
...
**Scoring scale (1-5) for category sections (Premise through Craft):**
- **5 — Excellent:** On par with a good theatrical release. This aspect could not be better. Mechanics functioning perfectly.
- **4 — Good:** A true strength with very little wrong. Most diagnostic questions addressed satisfactorily.
- **3 — Fair:** Did as much wrong as it did right. A mixed bag.
- **2 — Poor:** A clear weakness. Poorly executed.
- **1 — Very Poor:** Everything in this category is done wrong. The neglect makes the whole read challenging.

**Overall recommendation scale (scored and written last, after all ten categories):**
- **5 — Strong Recommend:** Flawless story mechanics with strongly persuasive impact. Mostly 4s and 5s individually, no more than one 3.
- **4 — Recommend:** Close to production-ready. Basic mechanics in place with maybe a couple exceptions. 3s, 4s, and 5s individually, nothing below 3.
- **3 — Consider:** Good with great potential. More strengths than weaknesses, but clear weaknesses exist. Mostly 3s and 4s, no 1s, possibly one or two 2s.
- **2 — Pass:** Would not make a good film in its present state. Roughly as many things wrong as right. Blend of 2s and 3s, possibly a 4 outlier.
- **1 — Strong Pass:** Most fundamentals not in place. Basic storytelling errors throughout. Mostly 2s and some 1s.

**Score distribution and calibration:**

Use the full 1-5 scale. A well-calibrated Consider typically contains a mix of 2s, 3s, and 4s — not a wall of 3s and 4s. A 2 is not a punishment; it is an accurate description of a category that is "a clear weakness, poorly executed." If the evidence supports a 2, assign a 2.

Do not let a strong premise inflate scores in execution categories. A screenplay can have a 4 Premise and a 2 Conflict if the promising concept is poorly served by the conflict mechanics. Score what the draft delivers, not what the concept promises.

Do not cluster scores. If your ten category scores contain no value below 3, ask yourself whether you have genuinely found nothing that qualifies as "a clear weakness" — or whether you are avoiding the score that matches your own analysis. Read your paragraph back: if the commentary describes a weakness but the score says Fair or Good, one of them is wrong.

The Overall recommendation must be consistent with the pattern of category scores. Refer to the individual score profiles defined above. If your scores contain a 2, the Overall cannot be Recommend. If your scores are mostly 3s with a couple of 4s and a 2, the Overall is Consider. Let the evidence drive the recommendation, not the other way around.
...
- A score of 4 or 5 must be earned by the text on the page, not by the concept's potential or the reader's goodwill toward the material.
...
A score of 4 (Good) requires that the material meaningfully distinguishes itself from its closest predecessors in both concept and execution. Acknowledging its influences openly, or executing familiar beats with polish, does not constitute originality — it constitutes competent derivation, which is a 3. A 4 demands that specific elements of the screenplay would surprise a viewer familiar with its genre and comparables.
...
### OVERALL
Score this LAST, after all other categories. Review the spread of your ten individual scores. **Open the Overall paragraph with a single framing sentence that identifies what the material is** — its genre, premise, and narrative identity — so that a reader encountering the coverage cold can orient themselves before the assessment begins. Example: "TITLE is a taut psychological thriller about a disgraced surgeon who discovers her hospital is harvesting organs." Then assess the strongest and weakest categories, presenting a holistic view of the material. The paragraph tone should match the recommendation level: Strong Recommend is entirely positive with at most a minor reservation; Pass is an even mix of strengths and weaknesses; Strong Pass identifies severe issues while remaining professional and constructive.

---

## IMPORTANT REMINDERS

1. Score the Overall section LAST. Assign all ten category scores first, then determine the Overall recommendation based on the pattern of scores. Overall is the final section in the output.
2. Your individual category scores must be internally consistent with the recommendation level. A "Recommend" should not contain any score below 3. A "Consider" should not contain a 1.
3. Each category paragraph must contain unique analysis — do not repeat the same observation across multiple sections.
4. Citations are mandatory in Structure, Character, Conflict, Dialogue, Pacing, Tone, Logic, and Craft. Minimum 3 per section.
5. Originality requires at least 2 comparable films referenced.
6. The total word count across all 11 comment sections should be approximately 900-1350 words.
7. Write in present tense throughout.
8. Never use first person.
9. Do not output a section header for the metadata fields. Do not output a section header for the cover page information.
10. Use the dual-format heading for all comment sections: CATEGORY NAME — Label (score).
